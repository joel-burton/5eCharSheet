...
class ModalEditor extends React.Component {
  ...
  render(props) {
    ...
    return <ModalWrapper {...this.props} title={modalTitle} 
      onSubmit={() => onSubmit({id: this.props.id, title: this.state.titleInput, school: this.state.magicSchoolInput, damage: this.state.damageInput, damageType: this.state.damageTypeInput, castingTime: this.state.castingTimeInput, properties: this.state.propertiesInput, range: this.state.rangeInput, duration: this.state.durationInput, description: this.state.descriptionInput, uses: this.state.usesInput, spellLevel: this.state.spellLevel })}>
        ...
        <View id={"durationInput"} style={[{ display: showDuration ? "flex" : "none" }]}>
          ...
          <TextInput id={"rangeInputBox"} style={styles.noteCardInput} onChangeText={text => this.changeText("durationInput", text)} placeholderTextColor={"#797979"} maxLength={35} placeholder={"Concentration, 10 minutes..."} autoGrow={false} value={this.state.durationInput} autoCapitalize={"words"} keyboardAppearance={"dark"} />
        </View>
